...
import { analyze, compileWorkbook, createDecision, dependencyFingerprint, evaluateGates, inferMappings, invalidateStaleDecisions, ingestCsvSources, loadMethodProfile, reconcile, xlsxToSourceFile } from "../../packages/core/dist/index.js";
...
function fingerprintFor(run) { const sourceHashes = [...new Set([...run.deal.claims, ...run.deal.cashEvents, ...run.deal.receivables, ...run.deal.glRecords].map((record) => record.lineage.sourceFileHash))].sort(); const mappingHash = createHash("sha256").update(JSON.stringify(run.mappings.map((mapping) => [mapping.id, mapping.reviewStatus, mapping.canonicalField]))).digest("hex"); const findingHash = createHash("sha256").update(JSON.stringify(run.findings.map((finding) => [finding.id, finding.status]))).digest("hex"); return dependencyFingerprint({ runId: run.id, methodVersion: run.profile.version, sourceHashes, mappingHash, assumptionHash: createHash("sha256").update(JSON.stringify(run.profile.recoveryRates)).digest("hex"), engineVersion: "r1-v0.2", findingHash }); }
