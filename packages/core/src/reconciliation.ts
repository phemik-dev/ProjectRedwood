...
export function reconcile(deal: CanonicalDeal, method: MethodProfile): { reconciliations: Reconciliation[]; findings: Finding[] } {
  ...
  const glCash = sum(deal.glRecords.map((record) => record.cash)); const glMatchedCash = sum(deal.glRecords.map((record) => record.matchedCash ?? record.cash)); const ar = sum(deal.receivables.map((item) => item.balance)); const glAr = sum(deal.glRecords.map((record) => record.ar));
  const controls = [
    ...
    control("ar-to-gl", "A/R snapshot to GL A/R", "Explicit A/R snapshot", "GL A/R", ar, glAr, method, deal.receivables.map((item) => item.lineage.sourceRowId).concat(deal.glRecords.map((record) => record.lineage.sourceRowId)))
  ];
